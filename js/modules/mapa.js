...
// Exportamos o mapa para que o roteirizador.js o possa usar
export let map;
...
let markersGroup, rainCircleLayer;
const markers = {};
const rainCircles = {};

export function iniciarMapa() {
  map = L.map("map", { zoomControl: false }).setView([-19.7, -47.0], 8);
  ...
  const SidebarToggleControl = L.Control.extend({
    ...
    onAdd: function (map) {
      ...
      container.innerHTML =
        '<a href="#" title="Ocultar/Mostrar Menu" role="button"><i data-lucide="menu" style="width: 18px; height: 18px;"></i></a>';
    ...
    .forEach((u) => {
      ...
      <div style="color: white; min-width: 220px;">
          ...
          <div class="popup-action-buttons">
              ...
              <button onclick="window.abrirDetalhes('${slug}', '${u.nome}')" class="btn-fotos-popup">Fotos</button>
          </div>
      </div>`);
...
// Ligando o input de pesquisa à função
setTimeout(() => {
  const filterInput = document.getElementById("filter");
  if (filterInput) filterInput.addEventListener("input", render);
}, 100);

// --- SISTEMA DE CLIMA (Isolado aqui para não criar dezenas de ficheiros) ---
